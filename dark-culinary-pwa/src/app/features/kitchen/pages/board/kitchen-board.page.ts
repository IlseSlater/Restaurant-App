...
export class KitchenBoardPage implements OnInit {
  ...
  get pending(): KitchenBoardItem[] {
    return this.items.filter((i) => i.status === 'PENDING');
  }

  get preparing(): KitchenBoardItem[] {
    return this.items.filter((i) => i.status === 'PREPARING');
  }

  get ready(): KitchenBoardItem[] {
  ...
  ngOnInit(): void {
    ...
    this.ws.connect();
    this.ws.joinCompanyRooms(companyGuid, ['kitchen']);
    ...
    this.ws.on<any>('order_created_kitchen').subscribe(() => this.loadOrders(companyGuid));
    ...
    this.api.get<any[]>('orders', { companyId }).subscribe({
      next: (orders) => {
        ...
        for (const order of orders ?? []) {
          ...
          if (orderIsTerminal) {
            // If the whole order is completed/served/cancelled, do not show any of its items on the kitchen board
            continue;
          }
          ...
          for (const item of order.items ?? []) {
            ...
            // As soon as an item is SERVED or COLLECTED, it should disappear from the kitchen board
  ...
  updateItemStatus(item: KitchenBoardItem, status: string): void {
    this.api
  ...
  minutesSince(item: KitchenBoardItem): number {
    return Math.floor((Date.now() - item.placedAt.getTime()) / 60000);
  }

  isSlaBreached(item: KitchenBoardItem): boolean {
