...
async function main() {
    ...
    if (existingSession) {
        ...
        participantIds = existingSession.participants.map((p) => p.id);
        creatorId = existingSession.participants.find((p) => p.isCreator)?.id ?? participantIds[0];
    ...
    if (orderExisting && orderExisting.items.length > 0) {
        ...
        orderId = order.id;
        const shareableItem = order.items.find((i) => i.isShareable);
        if (!shareableItem)
            throw new Error('No shareable item created');
...
//# sourceMappingURL=seed-social-dining.js.map